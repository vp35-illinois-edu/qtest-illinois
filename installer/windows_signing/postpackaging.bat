...
set BASE_PATH=%ROOT_PATH%\build\windows\win64\install

REM Define files to sign
set FILES_TO_SIGN=win64_qtest_Installer.exe
...
for %%F in (%FILES_TO_SIGN%) do (
    ...
    "%SCRIPT_PATH%SignTool\signtool.exe" sign /v /fd SHA256 /tr "http://timestamp.digicert.com" /td SHA256 /dlib "%SCRIPT_PATH%microsoft.trusted.signing.client\bin\x64\Azure.CodeSigning.Dlib.dll" /dmdf "%SCRIPT_PATH%metadata.json" %BASE_PATH%\%%F
    if %errorlevel% neq 0 (
